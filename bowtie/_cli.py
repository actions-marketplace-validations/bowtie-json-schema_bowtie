...
class _Report(click.File):
    ...
    def convert(  # type: ignore[reportIncompatibleMethodOverride]
        ...
        try:
            ...
            STDERR.print(error)
            ctx.exit(EX.DATAERR)
        except _report.DuplicateCase as err:
            error = DiagnosticError(
                code="duplicate-case",
                message="The Bowtie report contains a duplicate test case.",
                causes=[
                    f"{input.name} contains seq {err} more than once.",
                ],
                hint_stmt=(
                    "This likely indicates the report was produced incorrectly, "
                    "for example by concatenating two partial reports together. "
                    "Ensure you are passing in a single report generated by Bowtie."
                ),
            )
            STDERR.print(error)
            ctx.exit(EX.DATAERR)
